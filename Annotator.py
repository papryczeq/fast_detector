...
class Annotator:
    ...
    def human_readable_detection(detection):
        boxes = detection[:, 1:5].tolist()
        classes = detection[:, 6].astype(int).tolist()
        classes = list(map(coco_id_mapping.get, classes)) #changing ids to class names
    ...
    def gimme_bboxes(image):
        ...
        return human_readable_detection(detections_bs)